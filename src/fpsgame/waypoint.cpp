...
namespace ai
{
    ...
    static inline void invalidatewpcache(int wp)
    {
        ...
        {
            for(int i = 0; i< WPCACHE_DYNAMIC; i++) if(wp >= wpcaches[i].firstwp && wp <= wpcaches[i].lastwp) { invalidatedwpcaches |= 1<<i; return; }
    ...
    {
        for(int i = 0; i < NUMWPCACHES; i++) if(full || invalidatedwpcaches&(1<<i)) { wpcaches[i].clear(); clearedwpcaches |= 1<<i; }
    ...
    void buildwpcache()
    {
        for(int i = 0; i < int(NUMWPCACHES); i++) if(wpcaches[i].firstwp < 0)
    ...
    int closestwaypoint(const vec &pos, float mindist, bool links, fpsent *d)
    {
        ...
        for(int which = 0; which < int(NUMWPCACHES); which++) if(wpcaches[which].firstwp >= 0) for(curnode = &wpcaches[which].nodes[0], wpcachestack.setsize(0);;)
        {
