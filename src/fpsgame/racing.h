// racing.h: client and server state for racing gamemode
...
struct racingservmode : servmode
#else
...
struct racingclientmode : clientmode
#endif
{
...
#define COUNTDOWNSECONDS 3
    ...
    int getmaxcheckpoint() {
        ...
        loopv(ments) {
            entity& e = ments[i];
            if(e.type == N_RACINGCHECKPOINT && e.attr2 > m) {
                m = e.attr2;
            }
        }
    ...
    void update() {
        if (totalmillis > lastupdatecheck + 1000) {
            ...
            loopv(clients) {
                ...
                ci->state.racingrank = getrank(ci->clientnum);
                sendf(-1, 1, "ri7", N_RACINGINFO, ci->clientnum, ci->state.racingstate, ci->state.racinglaps, ci->state.racingcheckpoints, getracetime(ci), ci->state.racingrank);
            }
        }
    }
    ...
    int getracetime(clientinfo *ci) {
        loopv(raceinfos) if(raceinfos[i]->cn == ci->clientnum) {
            ...
            if (ci->state.racingstate == 2) {
                return raceinfos[i]->timefinished + (raceinfos[i]->timepenalties * 10000);
            } else {
                return totalmillis - ci->state.racingtime + (raceinfos[i]->timepenalties * 10000);
            }
        }
        return totalmillis - ci->state.racingtime;
    }

    void sendfinishannounce(clientinfo *ci) {
        int rank = ci->state.racingrank;
    ...
    void initplayer(clientinfo *ci) {
        ci->state.racinglaps = 0;
        ci->state.racingcheckpoints = 0;
        ci->state.racingrank = -1;
        ci->state.racingstate = 0;
        raceinfos.add(new raceinfo(ci->clientnum, (ci->state.state==CS_SPECTATOR ? -2 : -1), 0, 0));
        sendf(-1, 1, "ri7", N_RACINGINFO, ci->clientnum, 0, 0, 0, 0, -1);
    }
    ...
    bool canspawn(clientinfo *ci, bool connecting) {
        if (ci->state.racingstate == 2) return false;
        /* loopv(raceinfos) if(raceinfos[i]->cn == ci->clientnum){
    ...
    void died(clientinfo *target, clientinfo *actor) {
        ...
        if (actor && target->clientnum != actor->clientnum && (target->state.racinglaps > 0 || target->state.racingcheckpoints >= 2)) {
            if (target->state.racingcheckpoints >= 2) {
                target->state.racingcheckpoints -= 2;
            } else {
                target->state.racinglaps -= 1;
                target->state.racingcheckpoints = maxcheckpoint - 2 + target->state.racingcheckpoints;
            }
        }
    }
